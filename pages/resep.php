...
// Helper: parse stored recipe lists (bahan / cara_membuat) into clean arrays
function parse_list($text) {
    ...
    $text = str_replace(["\r\n", "\r"], "\n", $text);
    // First try splitting by two or more newlines (paragraphs separated by blank line)
    $parts = preg_split('/\n{2,}/', $text);
    // If that produced only one part but contains multiple lines, split by single newlines
    if (count($parts) <= 1) {
        $parts = preg_split('/\n+/', $text);
    }
    $out = [];
    foreach ($parts as $p) {
        $p = trim($p);
        // Remove any surrounding HTML tags for safety
        $p = strip_tags($p);
        if ($p !== '') $out[] = $p;
    }
    return $out;
        ...
        <div class="resep-grid">
            ...
            if ($result && $result->num_rows > 0) {
                while ($row = $result->fetch_assoc()) {
                    ...
                    // Split bahan dan cara membuat ke dalam array (robust parsing)
                    $bahan_array = parse_list($bahan);
                    $cara_array = parse_list($cara_membuat);
